...
class RBMAlgorithm(AlgoBase):
    ...
    def __init__(self, epochs=20, hiddenDim=100, learningRate=0.001, batchSize=100, sim_options={},ratingValue=10):
        AlgoBase.__init__(self)
        self.epochs = epochs
        self.hiddenDim = hiddenDim
        self.learningRate = learningRate
        self.batchSize = batchSize
        self.ratingValues=ratingValue
    ...
    def fit(self, trainset):
        ...
        trainingMatrix = np.zeros([numUsers, numItems, self.ratingValues], dtype=np.float32)
        ...
        rbm = RBM(trainingMatrix.shape[1], hiddenDimensions=self.hiddenDim, learningRate=self.learningRate, batchSize=self.batchSize, epochs=self.epochs,ratingValues=self.ratingValues)
        ...
        for uiid in range(trainset.n_users):
            ...
            recs = np.reshape(recs, [numItems, self.ratingValues])
            ...
            for itemID, rec in enumerate(recs):
                ...
                rating = np.average(np.arange(self.ratingValues), weights=normalized)
